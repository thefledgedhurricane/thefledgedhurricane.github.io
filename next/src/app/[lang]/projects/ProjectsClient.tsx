  ...
  },
  {
    ...
    content: () => (
      <>
        ...
        <Paragraph>
          Le test de Corsi (Corsi Block-Tapping Test) is un outil classique d'évaluation 
...
export default function ProjectsClient({ dict, lang }: ProjectsClientProps) {
  ...
  const categories = ['all', ...Array.from(new Set(projects.map(proj => proj.category)))];
  
  const filteredProjects = filterCategory === 'all' 
    ? projects 
    : projects.filter(proj => proj.category === filterCategory);

  const getStatusLabel = (status: string) => {
    const s = status.toLowerCase();
    if (s.includes('termin')) return p.status_completed || 'Terminé';
    if (s.includes('recherche')) return p.status_research || 'En Recherche';
    return p.status_opensource || 'Open Source';
  };
      ...
      <div className="min-h-screen bg-white pt-24">
        ...
        >
          <div className="mb-8">
            ...
            >
              <ArrowRight className="w-4 h-4 mr-2 rotate-180 group-hover:-translate-x-1 rtl:rotate-0 rtl:group-hover:translate-x-1 transition-transform" />
              {p.back_to_list || 'Back to projects'}
            </button>
          </div>
        ...
        <FadeIn>
          <div className="text-center mb-24">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-white border border-mckinsey-gray-200 rounded-full text-xs font-medium text-mckinsey-navy-800 mb-8 shadow-sm">
              ...
              {p.badge || 'Innovation & Development'}
            </div>
            
            <h1 className="text-5xl md:text-7xl font-light text-mckinsey-navy-900 mb-8 leading-[0.95] tracking-tight">
              {p.title || 'Featured'} <span className="font-normal text-transparent bg-clip-text bg-gradient-to-r from-mckinsey-navy-800 to-mckinsey-teal-600">{p.title_gradient || 'Projects'}</span>
            </h1>
            
            <p className="text-xl text-mckinsey-gray-600 font-light max-w-2xl mx-auto mb-12 leading-relaxed">
              {p.desc || 'Discover my research and development projects.'}
            </p>
            ...
            <div className="flex justify-center gap-3 flex-wrap">
              {categories.map((category) => (
                ...
                >
                  {category === 'all' ? (p.filter_all || 'All Projects') : category}
                </button>
              ))}
            </div>
          </div>
                  ...
                  {project.featured && (
                    <div className="mb-4">
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-mckinsey-teal-600">
                        <span className="w-1.5 h-1.5 bg-mckinsey-teal-500 rounded-full animate-pulse" /> {p.featured || 'Featured'}
                      </span>
                    </div>
                  )}
