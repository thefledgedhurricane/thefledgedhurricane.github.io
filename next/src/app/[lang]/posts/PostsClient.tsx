    ...
    return (
      ...
      >
        <div className="mb-8">
          ...
          >
            <svg className="w-4 h-4 mr-2 rotate-180 group-hover:-translate-x-1 rtl:rotate-0 rtl:group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 19l-7-7 7-7" />
            </svg>
            {p.back_to_list || 'Back to articles'}
          </button>
        </div>
    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <div className="relative h-[70vh] w-full overflow-hidden">
        ...
        <div className="absolute inset-0 z-20 flex flex-col justify-center items-center text-center px-6">
          <span className="text-mckinsey-teal-400 tracking-normal text-sm mb-6">{p.badge || 'JOURNAL'}</span>
          <h1 className="text-6xl md:text-8xl lg:text-9xl font-sans text-white mb-8 tracking-tight">
            {p.title || 'JOURNAL'}
          </h1>
          <p className="max-w-xl text-mckinsey-gray-200 font-normal text-lg leading-relaxed">
            {p.desc || 'Exploration of artificial intelligence, development and cognition.'}
          </p>
        </div>
      </div>
      ...
      <div className="max-w-[1800px] mx-auto px-6 py-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-x-8 gap-y-24">
          {posts.map((post, index) => {
            ...
            const monthYear = date.toLocaleDateString(lang === 'ar' ? 'ar-EG' : lang === 'fr' ? 'fr-FR' : 'en-US', { month: 'long', year: 'numeric' }).toUpperCase();
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <span className="text-xs tracking-normal text-mckinsey-navy-800/40">
                          {post.readingTime} {p.min_read || 'MIN READ'}
                        </span>
                      </div>
