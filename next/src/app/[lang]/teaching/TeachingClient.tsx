...
export default function TeachingClient({ dict, lang }: TeachingClientProps) {
  const t = dict.teaching || {};

  const difficultyLabels = {
    beginner: t.level_beginner || 'débutant',
    intermediate: t.level_intermediate || 'intermédiaire',
    advanced: t.level_advanced || 'avancé'
  } as const;

  const stats = [
    { value: courses.length, label: t.stats_courses || 'Cours complets', icon: BookOpen },
    { value: courses.reduce((total, course) => total + course.lessons.length, 0), label: t.stats_lessons || 'Leçons détaillées', icon: Layers },
    { value: courses.reduce((total, course) => total + course.estimatedHours, 0), label: t.stats_hours || 'Heures de contenu', icon: Clock },
    { value: 100, label: t.stats_free || '% Accès libre', icon: GraduationCap }
  ];
    ...
    <main className="min-h-screen bg-white selection:bg-mckinsey-teal-100 selection:text-mckinsey-navy-900">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full max-w-7xl pointer-events-none">
          <div className="absolute top-20 left-20 w-96 h-96 bg-mckinsey-teal-50/40 rounded-full blur-3xl mix-blend-multiply" />
          <div className="absolute top-40 right-20 w-96 h-96 bg-mckinsey-navy-50/40 rounded-full blur-3xl mix-blend-multiply" />
        </div>
          ...
          <FadeIn>
            <div className="text-center max-w-3xl mx-auto mb-16">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-white border border-mckinsey-gray-200 rounded-full text-xs font-medium text-mckinsey-navy-800 mb-8 shadow-sm">
                ...
                {t.badge || "Espace d'Apprentissage"}
              </div>
              <h1 className="text-5xl lg:text-7xl font-light text-mckinsey-navy-900 mb-6 tracking-tight">
                {t.title || 'Excellence'} <span className="font-normal text-mckinsey-teal-600">{t.title_gradient || 'Pédagogique'}</span>
              </h1>
              <p className="text-xl text-mckinsey-gray-600 leading-relaxed">
                {t.desc || "Une approche moderne de l'enseignement."}
              </p>
              <Link
                href={`/${lang}/teaching`}
              ...
              >
                {t.guided_track || 'Choisir un parcours guidé'}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {courses.map((course, idx) => (
              <FadeIn key={course.id} delay={idx * 100}>
                <Link href={`/${lang}/teaching/modules/${course.id}`} className="block h-full">
                  <div className="group h-full bg-white rounded-2xl border border-gray-100 hover:border-mckinsey-teal-200 hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col">
                    <div className={`h-2 bg-gradient-to-r ${trackColors[course.category as keyof typeof trackColors] || 'from-gray-500 to-slate-500'}`} />
                    <div className="p-8 flex flex-col flex-grow">
                      <div className="flex justify-between items-start mb-6">
                        ...
                        <span className="px-3 py-1 bg-gray-50 rounded-full text-xs font-medium text-gray-600 uppercase tracking-wider">
                          {difficultyLabels[course.level as keyof typeof difficultyLabels]}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-6 border-t border-gray-100">
                        <div className="flex items-center gap-4 text-sm text-gray-500">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {course.lessons.length} {t.lessons_count_label || 'leçons'}
                          </span>
                        </div>
                        <ArrowRight className="w-5 h-5 text-mckinsey-teal-500 transform group-hover:translate-x-1 rtl:rotate-180 rtl:group-hover:-translate-x-1 transition-transform" />
                      </div>
                    </div>
                  </div>
                </Link>
